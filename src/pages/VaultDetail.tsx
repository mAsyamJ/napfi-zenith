...
import { 
  ...
  ArrowDownRight, RefreshCw, Settings, ChevronDown,
  ExternalLink, Code, Sparkles, Clock, Wallet, Bell
} from "lucide-react";
        ...
        <div className="p-4 md:p-6 lg:p-8">
          ...
          <Card className="bg-card border-border mb-6 shadow-sm">
            ...
            <CardContent className="space-y-4">
              {/* Strategy 1 */}
              <details className="bg-muted/30 rounded-lg border border-border" open>
                <summary className="p-4 cursor-pointer hover:bg-muted/50 transition-colors flex justify-between items-center">
                  <div className="flex items-center gap-4">
                    <div className="bg-primary/20 p-2 rounded-full">
                      <Sparkles className="h-5 w-5 text-primary" />
                    </div>
                    <div>
                      <h3 className="font-medium">{vault.platform} {vault.name} - Core Strategy</h3>
                      <p className="text-sm text-muted-foreground">Multi-asset LP farming</p>
                    </div>
                    <Badge className="bg-success/20 text-success">Active</Badge>
                  </div>
                  <div className="flex items-center gap-8">
                    <div>
                      <p className="text-right font-medium">{(vault.apy * 0.65).toFixed(1)}%</p>
                      <p className="text-xs text-muted-foreground">Base APY</p>
                    </div>
                    <div>
                      <p className="text-right font-medium">65%</p>
                ...
                <div className="px-4 pb-4">
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                    ...
                    <div className="bg-card p-4 rounded-lg border border-border">
                      <h4 className="text-sm font-medium mb-3">Strategy Details</h4>
                      <div className="space-y-2">
                        <div className="flex justify-between text-sm">
                          <span className="text-muted-foreground">Risk Score</span>
                          <div className="flex items-center gap-2">
                            <span className="capitalize">{vault.risk}</span>
                            <div className="w-16 h-2 bg-muted rounded-full">
                              <div 
                                className={`h-full rounded-full ${
                                  vault.risk === "low" ? "bg-success w-1/3" :
                                  vault.risk === "medium" ? "bg-warning w-2/3" :
                        ...
                        <div className="flex justify-between text-sm">
                          ...
                          <span>${(vault.tvl * 0.65 / 1000000).toFixed(2)}M</span>
                        </div>
                        <div className="flex justify-between text-sm">
                    ...
                    {/* Performance Metrics */}
                    <div className="bg-card p-4 rounded-lg border border-border lg:col-span-2">
                      ...
                      <div className="grid grid-cols-3 gap-3 mb-4">
                        <div className="bg-muted/50 p-3 rounded-lg">
                          ...
                          <p className="text-lg font-medium">{(vault.apy * 0.65).toFixed(1)}%</p>
                        ...
                        <div className="bg-muted/50 p-3 rounded-lg">
                          <p className="text-xs text-muted-foreground">Weekly APY</p>
                          <p className="text-lg font-medium">{(vault.apy * 0.62).toFixed(1)}%</p>
                        ...
                        <div className="bg-muted/50 p-3 rounded-lg">
                          <p className="text-xs text-muted-foreground">Monthly APY</p>
                          <p className="text-lg font-medium">{(vault.apy * 0.64).toFixed(1)}%</p>
                          <p className="text-success text-xs flex items-center">
                            <ArrowUpRight className="h-3 w-3 mr-1" />
                            +1.2% (30d)
                          </p>
                      ...
                      <div className="bg-muted/30 rounded-lg p-3 border border-border">
                        <div className="flex justify-between mb-2">
                          <h5 className="text-xs font-medium">Historical Performance</h5>
                          <div className="flex gap-2">
                        ...
                        <div className="h-[140px]">
                          <Chart
                            ...
                            height="100%"
                            width="100%"
                            series={[{ name: "Strategy APY", data: [11.2, 11.8, 11.5, 12.0, 11.7, 12.3, 12.3] }]}
              ...
              <details className="bg-muted/30 rounded-lg border border-border">
                <summary className="p-4 cursor-pointer hover:bg-muted/50 transition-colors flex justify-between items-center">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="font-medium">Yearn {vault.name} Optimizer</h3>
                      <p className="text-sm text-muted-foreground">Dynamic yield optimizer</p>
                    </div>
                    <Badge className="bg-success/20 text-success">Active</Badge>
                  </div>
                  <div className="flex items-center gap-8">
                    <div>
                      <p className="text-right font-medium">{(vault.apy * 0.25).toFixed(1)}%</p>
                      <p className="text-xs text-muted-foreground">Base APY</p>
                    </div>
                    <div>
                      <p className="text-right font-medium">25%</p>
                      <p className="text-xs text-muted-foreground">Allocation</p>
                    </div>
                    <ChevronDown className="h-5 w-5" />
                  </div>
              ...
              <details className="bg-muted/30 rounded-lg border border-border">
                <summary className="p-4 cursor-pointer hover:bg-muted/50 transition-colors flex justify-between items-center">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="font-medium">{vault.name} Staking Rewards</h3>
                      <p className="text-sm text-muted-foreground">Passive staking rewards</p>
                    </div>
                    <Badge className="bg-success/20 text-success">Active</Badge>
                  </div>
                  <div className="flex items-center gap-8">
                    <div>
                      <p className="text-right font-medium">{(vault.apy * 0.1).toFixed(1)}%</p>
                      <p className="text-xs text-muted-foreground">Base APY</p>
                    </div>
                    <div>
                      <p className="text-right font-medium">10%</p>
                      <p className="text-xs text-muted-foreground">Allocation</p>
                    </div>
                    <ChevronDown className="h-5 w-5" />
                  </div>
